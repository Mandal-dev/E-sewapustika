        ...
        try {
            ...
            if (!empty($keyword)) {
                ...
                $query->where(function ($q) use ($keyword) {
                    $q->whereRaw('LOWER(t4.police_name) LIKE ?', ["%{$keyword}%"])
                        ->orWhereRaw('LOWER(t4.buckle_number) LIKE ?', ["%{$keyword}%"])
                        ->orWhereRaw('LOWER(t4.designation_type) LIKE ?', ["%{$keyword}%"])
                        ->orWhereRaw('LOWER(t3.city_name) LIKE ?', ["%{$keyword}%"])
                        ->orWhereRaw('LOWER(t2.district_name) LIKE ?', ["%{$keyword}%"])
                        ->orWhereRaw('LOWER(t1.state_name) LIKE ?', ["%{$keyword}%"]);
                });
            }

            $polices = $query->paginate($perPage)->appends(['keyword' => $request->keyword]);
            ...
            return view('sewa_pustika.index', compact('polices', 'keyword'));
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Something went wrong. Please try again later.');
        }
    }
